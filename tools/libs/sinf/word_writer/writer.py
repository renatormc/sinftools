...
import pythoncom
from sinf.word_writer.jinja_env_filters import filters
from sinf.word_writer.jinja_env_functions import global_functions
import xml.etree.ElementTree as ET
import os
from sinf.word_writer.helpers.writer import set_defaults_attrib, to_points, remove_br
import codecs
import json
...
class Writer:
    ...
    def add_filter(self, filter, name=None):
        ...
        self.jinja_env.filters[name] = filter

    def add_global_function(self, function, name=None):
        if not name:
    ...
    def format_range(self, range, el):
        if el.attrib['font-weight']:
            ...
            for part in parts:
                getattr(formatter.FontWeight, part)(range)
        if el.attrib['alignment']:
            formatter.alignment(range, el)
        if el.attrib['font-size']:
            formatter.font_size(range, el.attrib['font-size'])
        if el.attrib['font-name']:
            formatter.font_name(range, el.attrib['font-name'])
        if el.attrib['font-color']:
            formatter.font_color(range, el.attrib['font-color'])
    ...
    def render_word(self, template, **context):

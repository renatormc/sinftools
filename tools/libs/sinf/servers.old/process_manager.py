...
from sinf.servers.helpers_database import filter_depends_on
import subprocess
import shlex
import psutil
...
import stat
import errno
from sinf.servers.iped_config_reader import IpedConfigReader
...
class ProcessManager:
    ...
    def kill_process(self, proc):
        ...
        try:
            ...
            if create_time == proc.start:
                subprocess.check_output(["taskkill", "/F", "/T", "/IM", str(proc.pid)])
                # p.kill()
                self.check_process()
    ...
    def startup_iped_procs(self):
        ...
        n = self.db_session.query(Process).filter(Process.status == 'PROCESSANDO', Process.type == "IPED").count()
        if n == 0:
    ...
    def startup_image_procs(self):
        n = self.db_session.query(Process).filter(Process.status == 'PROCESSANDO', Process.type == "Imagem").count()
        if n == 0:
            query = self.db_session.query(Process).filter(Process.status == 'AGUARDANDO', Process.type == "Imagem")
            proc = query.order_by(
                Process.start_waiting.asc()).first()
            if proc:
                self.init_process(proc)
    ...
    def startup_other_procs(self):
        query = self.db_session.query(Process).filter(Process.status == 'AGUARDANDO', Process.type == "Outro")
                ...
                if proc.telegram:
                    try:
                        ...
                        if proc.status == "FINALIZADO":
                            servers_com.send_message(proc.perito, f"Seu processo \"{proc_name}\" finalizou com sucesso!")
                        if proc.status == "ERRO":
                            servers_com.send_message(proc.perito, f"Infelizmente houve um erro no seu processo \"{proc_name}\"")
                    except:
                        pass
    ...
    def set_blocking_user(self, user):
        if not user:
            print("ASDFFFFF")
            self.db_session.query(Document).filter(Document.key == "blocking_user").delete()
            self.db_session.commit()
            return
        doc = self.db_session.query(Document).filter(Document.key == "blocking_user").first()
        if not doc:
            doc = Document()
    ...
    def get_blocking_user(self):
        ...
        if doc:
            value = doc.value
            value['timestamp'] = datetime.strptime(value['timestamp'], "%d/%m/%Y %H:%M:%S")
            return value

...
class ProcessManager(object):
    ...
    def get_available_iped(self):
        ipeds_using = [proc.iped_folder for proc in
                       self.db_session.query(Process).filter(Process.status == "PROCESSANDO").all()]
    ...
    def check_process(self):
        ...
        # processess = self.db_session.query(Process).order_by(Process.queue_order.desc(), Process.start_waiting.asc()).all()
        ...
        # n = config.max_simultaneous - self.db_session.query(Process).filter(Process.status == 'PROCESSANDO').count()
        # if n < 0:
        #     n = 0
        # procs = self.db_session.query(Process).filter(Process.status == 'AGUARDANDO').order_by(

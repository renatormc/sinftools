...
begin
  ...
  begin
    ...
    begin
      try
        PathTo := TPath.Combine(Form1.ExtractFolder, Files[J]);
        PathFrom := TPath.Combine(Form1.Drive, Files[J]);
...
//        ShowMessage(PathFrom + ' ' + IntToStr(FileLength));
        ...
        on E: Exception do
        begin
          showmessage('Houve um erro ao tentar copiar os arquivos. Siga o tutorial e execute a cópia de forma manual.');
  ...
  Path := '"' + TPath.Combine(Form1.ExtractFolder, 'dados.exe') + '"';
  ...
  ShellExecute(Handle, 'open', 'explorer', PChar(Form1.DestinationFolder), '',
    SW_SHOWNORMAL);
  ShowMessage('Processo finalizado com sucesso! Os dados estão disponíveis pasta ' + '"' + Form1.DestinationFolder + '"');
  Application.Destroy;
end;
    ...
    try
      try
        ...
        begin
          ...
          ExtractFolder := TPath.Combine(DestinationFolder, 'dados_compactados');
          CreateDir(ExtractFolder);
          FileSetAttr(ExtractFolder, faHidden);
          Memo1.Visible := False;
...
//  Drive := 'G:\';
  ...
  Midias.LoadFromFile(Drive + '\.sinf\midias.txt');

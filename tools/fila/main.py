...
from sinf.servers.database import db_session, init_db
...
import shutil
import subprocess
from sinf.servers.logger import logging, get_log_tail
from sinf.servers.com import request_check_process
from sinf.servers.user_manage import who_is_connected
...
class Window(QMainWindow):
    ...
    def setup_ui(self):
        self.setWindowIcon(
            QIcon('{}\\resources\\icone.png'.format(config_.app_dir)))
    ...
    def create_menus(self):
        ...
        self.toggle_blocking_queue_action = QAction("Bloquear/Desbloquear fila")
    ...
    def open_sqlite(self):
        self.process_manager.exec(['s-dbb', str(config_.sqlite_path)])

    def open_config(self):
        self.process_manager.exec(['s-np', str(config_.config_path)])
    ...
    def set_block_message(self, message):
        if message:
            ...
            self.lbl_block_message.setStyleSheet("color: red; font-size: 20pt;")
    ...
    def update_blocking_state(self):
        ...
        if user:
            ...
            self.set_block_message(f"Fila bloqueada por {user['user']} desde {timestamp_str}")
        else:
    ...
    def toggle_blocking_queue(self):
        ...
        if user:
            ...
            if who and who['name'] != "Alguém":
                ...
                text, okPressed = QInputDialog.getText(self, "Perito","Seu nome:", QLineEdit.Normal, "")
                if okPressed:
                    if len(text.strip()) < 4:
                        self.show_error("O nome precisa ter no mínimo 4 caracteres")
                        return
    ...
    def create_new_script(self):
        ...
        if dialog.ok_clicked:
            ...
            script = config_.scripts_template[type]['script']
            to_script = config_.scripts_folder / f"{name}.bat"
            i = 1
            while to_script.exists():
                to_script = config_.scripts_folder / f"{name}_{i}.bat"
                i += 1
            from_script = config_.app_dir / f"cmd/scripts/{script}"
            shutil.copy(from_script, to_script)
            process = Process()
            process.script = str(to_script.absolute())
            process.perito = perito
            process.type = config_.scripts_template[type]['process_type']
    ...
    def open_logfile(self):
        self.process_manager.exec(['s-np', str(config_.logfile)])
    ...
    def connections(self):
        ...
        self.toggle_blocking_queue_action.triggered.connect(self.toggle_blocking_queue)
